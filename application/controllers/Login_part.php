...
class Login_part extends CI_Controller {
	...
	{	
		switch ($this->session->userdata('perfil')) {
			case '':
				...
				$this->load->view('login_part_view',$data);
				break;
			case 'administrador':
			...
			case 'formador':
				...
				break;	
			case 'participant':
				redirect(base_url().'index.php/Admin_part');
				break;
			default:		
				$data['titulo'] = 'Login con roles de usuario en codeigniter';
				$this->load->view('login_part_view',$data);
	...
	public function new_user()
	   ...
	   //          $this->form_validation->set_rules('password', 'Password', 'required');
	 		...
	 		// 	//validamos que se introduzcan los campos requeridos con la función de ci required
				...
				{
					//$this->index();
					redirect(base_url().'index.php/Login_part');
				}else{
					$username = $this->input->post('username');
					$password = sha1($this->input->post('password'));
					$check_user = $this->login_part_model->login_user($username,$password);
					if($check_user == TRUE)
					{
						$data = array(
		                'is_logued_in' 	=> 		TRUE,
